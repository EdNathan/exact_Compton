      subroutine srf_nonlimit(prob, df, nmaxp, limit,
     &                        fSInd, fLen, check)
...
c
c     Input arguments: 
c         prob:   The SRF probabilities, given in coordinated (final energy, initial energy)
...
c     final energy.   (If fortran 95, could use MAXLOC)
      do eini=1, nmaxp
         do efin=2, nmaxp  
            if(prob(efin, eini) .GT. prob(maxind(eini), eini))then
               maxind(eini) = efin
            endif
         enddo
         ...
         problimit(eini) = prob(maxind(eini), eini) * limit
...
c        the probability would be below the limit
         do efin = 1, maxind(eini)
            if(prob(efin, eini) .GT. problimit(eini))then
               iSInd(eini) = efin
               exit
            endif
         enddo
