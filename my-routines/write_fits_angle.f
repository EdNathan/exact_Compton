...
c-----------------------------------------------------------------------
      ...
      subroutine create_fits_angle(filename)
      implicit none
      character* (*) filename
!     Internal variables
...
c Delete the file if it already exists
      call deletefile_angle(filename,status)
...
c-----------------------------------------------------------------------
      ! create fits file with one extension and  columns 
      subroutine write_param_angle(temp_dim, en_dim, temp, en, filename,
     & mgi,smit,agt )
      implicit none
...
!     Filling the columns 
     ...
     &       en, status)
            colnum = 3
            call ftpcld(unit, colnum, rownum, 1, mgi,
     &       smit, status)
            colnum = 4
            call ftpcld(unit, colnum, rownum, 1, mgi,
     &       agt, status)
...
!IMPORTANT: this routine leaves the fits file opened
      ...
      subroutine add_HDU_angle(temp_dim, en_dim,filename, unit)
      implicit none

      integer :: unit, temp_dim, en_dim,mgi
      character* (*) filename
...
c-----------------------------------------------------------------------
      subroutine add_row_HDU_angle(n, nmaxp, out_en_dim, out_en_ind,mgi,
     & kn_cross, srf, unit)
      implicit none
      
      integer          :: unit, n, nmaxp, out_en_dim, out_en_ind,mgi
      double precision :: srf(mgi,nmaxp)
      double precision :: kn_cross
...
!Fill the column with arrays of different size
            call ftpcld(unit, colnum, rownum, 1, out_en_dim*8,
     &           srf, status)
...
c-----------------------------------------------------------------------
      subroutine deletefile_angle(filename,status)
...
c-----------------------------------------------------------------------
      subroutine printerror_angle(status)
